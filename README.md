...
- 모델 성능<br>
초기 모델 대비 데이터 증강 및 라벨링 기준 고도화를 거쳐 mAP50 지표를 약 25% 향상시켰습니다. 최종 모델 기준 75.9%를 달성하여, 다양한 데스크 환경에서도 안정적인 객체 탐지 성능을 확보했습니다.
...
- AI 모델 성능 검증
  ...
  - 지속적인 학습 실험 결과를 분석하여 성능이 개선되지 않는 객체는 제외하거나 라벨링 기준을 재정립함으로써, 최종 모델의 mAP50 지표를 약 25% 향상시켰습니다.

- 서비스 로직 및 파이프라인 설계 <br>
  - YOLO의 좌표 분석 결과와 인체공학 가이드라인 수치를 결합하여, 사용자에게 실직적인 도움을 주는 자연어 리포트 생성 프롬프트를 구현했습니다.
  - YOLO 객체 탐지 -> 인체공학적 수치 계산 -> GPT 해석으로 이어지는 전 과정을 하나의 파이프라인으로 연결하여 실시간 서비스가 가능한 형태로 완성하였습니다.

- UI/UX 설계 및 시스템 구현<br>
  - 복잡한 분석 결과를 한눈에 이해하기 쉽게 시각화하고, 분석 단계를 간소화하여 사용자 편의성을 높였습니다. 특히 인공지능 분석 중에는 실시간 진행 상황을 안내하고 오류 발생 시 즉각 대응하는 화면을 설계하여, 사용자가 끊김 없이 서비스를 이용할 수 있도록 했습니다.
